...
namespace XComosMobile.Pages.maintenance
{
    ...
    public partial class WorkPackages : PageTemplate
    {

        public WorkPackages(bool keepElements = false)
        {
            InitializeComponent();
        ...
        private async void TasksPage_Appearing(object sender, EventArgs e)
        {
            if (first_time)
            {
                this.ShowSpinner(Services.TranslateExtension.TranslateText("loading_wp"));
        ...
        private async Task<bool> LoadWorkPackages()
        {
            ...
            try
            {
                ...
                if (o == null)
                {
                    try
                    {
                        o = await m_ComosWeb.GetObjectBySystemFullName(projectdata.SelectedLayer, Constants.QueryWorkPackagesSystemFullName);
                    }
                    catch (Exception ex)
                    {
